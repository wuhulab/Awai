"""
AutoAPI - 冷却管理器模块
管理 API key 和模型的冷却状态，支持 429 错误处理和轮询机制
"""
...
class CooldownManager:
    ...
    def configure(self, settings: list):
        """
        配置冷却管理器
    ...
    def trigger_cooldown(self, api_key: str, model: str, duration: Optional[int] = None):
        """
        触发冷却
        ...
        logger.warning(f"触发冷却: key={api_key[:15]}..., model={model}, 冷却时长={duration}s, 直到={time.strftime('%H:%M:%S', time.localtime(cooldown_until))}")
    ...
    def get_available_key(self, api_keys: list, model: str) -> Optional[str]:
        """
        ...
        available_keys = [
            ...
            if not self.is_in_cooldown(key, model)
        ]
    ...
    def clear_cooldown(self, api_key: Optional[str] = None, model: Optional[str] = None):
        """
        清除冷却状态
